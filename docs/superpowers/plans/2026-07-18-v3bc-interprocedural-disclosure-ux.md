...
## Task B7 (CONDITIONAL): Cross-contract-call sink (K3/K4/K5) — **SKIPPED (Q7 = NO)**

> **RESOLVED 2026-07-18 — SKIPPED.** `task-Q7-findings.md` concluded **NO**: every cross-contract-call program in compactc 0.31.1 is rejected by a `cross-contract calls are not yet supported` feature gate (exit 255), so no compiling accept fixture exists and no reject+accept differential pair is possible. Contract-call (K3/K4/K5) stays a fail-closed **amber advisory** with `native_confirms: false`, exactly as v3a ships it. B5's reject-direction parity assertion ensures corpus cross-contract programs fail-closed to amber (never silent green). Empirical nuance recorded in the ledger: the WPP K5 sink *does* fire (banner before the gate reject), and K3/K4 (witness-derived contract reference) is type-system-unreachable. **Do not implement this task.**

**Original gate (not met):** implement this task ONLY IF `.superpowers/sdd/task-Q7-findings.md` concludes YES — i.e. an impure, non-constructor circuit making a `contract-call` with a witness-derived argument compiles far enough that local `compactc` 0.31.1 emits the WPP disclosure banner (making the sink differentially testable). **If Q7 concludes NO** (feature rejected regardless), SKIP this task: keep `contract-call` a fail-closed amber advisory (as v3a does), record the empirical evidence in `progress-v3bc.md`, and note in the PR that the handover's amber-gating was empirically upheld. **sonnet rust-dev; OPUS reviewer** (new confirmed-leak sink). Dispatch `compact-core:compact-dev` for the fixtures.
