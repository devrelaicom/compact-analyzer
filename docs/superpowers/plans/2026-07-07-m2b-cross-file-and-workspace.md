...
- **E5 (Task 12 watched-files test is non-guarding — startup-crawl race).** Task 12's `watched_file_creation_updates_the_index` test writes `added.compact` *after* `initialize_root` and then relies on the `didChangeWatchedFiles` handler to index it — but the Task-10 eager crawl (`discover_and_index`, synchronous in `run()` before the main loop) can complete *after* the test's `fs::write` and index the file on its own, so the test PASSES even if the watcher handler is broken or deleted (empirically confirmed: it passed 5/5 pre-implementation, and failed only when a sleep forced the crawl to finish first). Fix: make it deterministic with a crawl-completion barrier — (1) query `workspace/symbol "addedThing"` and assert EMPTY (a served response proves the server reached its main loop, i.e. the crawl already ran and found nothing), (2) then `fs::write` + notify the change, (3) then re-query and assert present (now attributable only to the watcher). This was verified to genuinely guard the feature by neutering `did_change_watched_files` and observing the rewritten test FAIL. Test-only change; production code unaffected.
