...
- [ ] **Step 2: Write one accept + one reject fixture per source/sink/sanitizer rule.** Each is minimal and derived from the R0 index / corpus `examples/`. Verify each `discloses` value against local `compactc` in Step 3:
  ...
  - ~~`contract_call_witness_leak` / `contract_call_no_witness`~~ — **NOT SHIPPED (R2 finding, 2026-07-16, compiler-verified):** cross-contract calls are **unconditionally rejected in compactc 0.31.1** with a second error `"cross-contract calls are not yet supported"` (doc: "Compact 1.0 does not fully implement declarations of contracts and the cross-contract calls they support"), independent of witness content. Consequences: (a) no `discloses:false` variant can ever compile to exit 0; (b) the WPP *does* fire on the witness-carrying call (K3/K5, nature `"contract call argument 1"` — confirms the pass is contract-call-aware) but only via a **two-file composable build** (impl + caller in sibling output dirs), which the single-file `compiler_discloses`/`compile_file` harness cannot drive. So contract-call is **version-gated like `emit`** (spec Rev-2.1 policy): spec'd, deferred, not differential-testable against 0.31.1. It is NOT in `FIXTURES`. See Task A8 for how its advisory is tested instead.
...
**Files:** Modify: `crates/analyzer-core/src/disclosure/interp.rs`, `leaks.rs`, `mod.rs`; Create: `crates/compact-analyzer/tests/fixtures/disclosure/struct_field_projection_ok.compact` (+ FIXTURES entry).
...
- [ ] **Step 0 (REQUIRED before the A8 corpus gate — A3 review deferral): make member projection precise at the A3 member arm.** Currently `interp_member` projects precisely only for struct-LITERAL bases; a **name-bound** struct falls to a flat `Atomic(union of ALL fields)`, which over-taints a clean field → a FALSE POSITIVE confirmed leak once sinks exist. Fix AT THE MEMBER ARM (`interp_member` in `interp.rs`), NOT via A4 env recovery (granularity is destroyed if you wait — the `Multiple` base already holds per-field taint; you only need field-name→index). Resolve the base expression's static struct type via `type_at` (`infer.rs:588`, reachable from `ctx: &InterpCtx`) to get the field order, then project the matching `Multiple` child. Keep the fail-closed `Atomic` union ONLY for genuinely-unknown/unresolvable base types (still advisory-worthy — prefer advisory over a red leak on true uncertainty). Add fixture `struct_field_projection_ok.compact` (a name-bound struct with a witness field AND a clean field; return/ledger-write the CLEAN field; compiler ACCEPTS ⇒ `discloses:false`, `native_confirms:false`) — this is RED against the pre-fix over-taint (native would confirm-leak) and GREEN after. Also fix the Minor: `interp_stmt`'s `Assign` should interpret only the RHS as a value (not the Member/Index LHS write target), to avoid a spurious projection advisory.
...
- [ ] **Step 5: Failing test for the deferred-sink advisory.** ⚠️ **R2 finding (2026-07-16):** there is NO shippable contract-call differential fixture — cross-contract calls are unconditionally rejected in 0.31.1 ("not yet supported") and only compile via a two-file composable build the harness can't drive. So do NOT write `impure_call_leak.compact` as a `compiler_discloses`-cross-checked fixture. Instead: (a) **primary** — demonstrate the advisory channel end-to-end via a fail-closed surface that IS testable in 0.31.1 (an **unknown-native call** or an **unhandled expression** — reuse A6/A3's advisory sites), asserting a `U3100`/`Severity::Advisory` diagnostic, NOT a false-positive E-leak; and (b) **contract-call specifically** — a `analyzer-core`-level unit test that constructs/parses a contract-call node and asserts the interpreter emits the deferred-sink advisory (never an E-leak), without routing through `compile_file`. Re-confirm at A8 execution time whether a contract-call node even parses standalone in the analyzer's item-tree (a `ledger t: Callee;` field may resolve to `Unknown` → already fail-closed); adjust the unit test to whatever node actually reaches the interpreter.
- [ ] **Step 6: Implement** the deferred-sink advisory (general fail-closed emitter + the contract-call site if it reaches the interpreter); run → PASS.
