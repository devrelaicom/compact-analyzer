...
- **Task 3 / V6 dist fixtures (HARVESTED — Tasks 5/6/11 hardcode these; from `dist plan` + a real host `dist build` on cargo-dist 0.32.0):**
  - **Archive names (versionless):** `compact-analyzer-aarch64-apple-darwin.tar.gz`, `compact-analyzer-x86_64-apple-darwin.tar.gz`, `compact-analyzer-x86_64-unknown-linux-gnu.tar.gz`, `compact-analyzer-x86_64-pc-windows-msvc.tar.gz`. (`artifactName(target)` returns exactly these — the draft's `version` param stays dropped.)
  - **Tag format:** `v{version}` → `v0.1.0`. Workflow trigger glob `**[0-9]+.[0-9]+.[0-9]+*`.
  - **Checksums:** per-artifact `<archive>.tar.gz.sha256` whose content is `<64-hex-sha256> *<bare-archive-filename>` (leading-space-then-`*` binary-mode marker, bare filename no path); PLUS an aggregate `sha256.sum` (unified-checksum, same line shape); PLUS `dist-manifest.json`. Task 6 parses the per-artifact `.sha256` (or the manifest) — mind the `*` prefix + bare filename when parsing.
  - **Installers:** `compact-analyzer-installer.sh`, `compact-analyzer-installer.ps1`. **Homebrew formula:** `compact-analyzer.rb`. Plus `source.tar.gz`(+`.sha256`). 14 artifacts total, 1 app.
  - **IN-ARCHIVE BINARY PATH IS NESTED (Task 6 extractor MUST strip the leading dir):** not top-level. Unix triples: `<archive-stem>/compact-analyzer` (e.g. `compact-analyzer-aarch64-apple-darwin/compact-analyzer`). **Windows: `compact-analyzer-x86_64-pc-windows-msvc/compact-analyzer.exe`** (the `.exe` suffix — Task 5 `binaryName` + Task 6 must special-case it). Corrects the plan's Task-6 Context ("top level or under a single directory") to definitively "under a single directory == the archive stem". Archives also contain `README.md` + `LICENSE` alongside the binary.
- **Task 9 / V1 `vscode-languageclient` 10.1.0 API shapes (read from the installed `.d.ts` — the expected heavy-hitter):**
  - **Import from the `/node` subpath:** `import { LanguageClient, type Executable, type LanguageClientOptions } from "vscode-languageclient/node"` — the package's `.` export (Node16 resolution → `lib/common/api`) is browser-safe and LACKS `LanguageClient`.
  - **Constructor:** `new LanguageClient(id, name, serverOptions, clientOptions)` (the 5-arg overload; the 4-arg `(name, serverOptions, clientOptions, forceDebug?)` also exists).
  - **stdio server:** an `Executable` `{ command, args: [] }` with NO `transport` field → stdio is the default (confirmed in `main.js`). `TransportKind.stdio` is NOT needed. `ServerOptions` accepts an `Executable` directly.
  - **`outputChannel` MUST be a `LogOutputChannel`** (created via `window.createOutputChannel(name, { log: true })` — the plain overload returns `OutputChannel` and does NOT type-check). Server STDERR is auto-piped to it (`pipeStderrToLogOutputChannel`) — pass `outputChannel` and the startup toolchain line appears automatically. A CLIENT-supplied channel is NOT disposed by `client.dispose()`/`stop()` (`_disposeOutputChannel=false`) — the extension owns it (push to `context.subscriptions`).
  - **Lifecycle:** `start(): Promise<void>` resolves AFTER `initialize` completes; `stop(timeout?)`; `dispose(timeout?)` = stop + full cleanup; `state` getter (`Stopped=1, Running=2, Starting=3, StartFailed=4`). A `StartFailed` client cannot be `stop()`ped (throws) and its `start()` returns the same rejected promise — so on a start failure you must `dispose()` and construct a FRESH client (do NOT reuse or call `client.restart()`, which reuses the original `serverOptions` and would skip re-acquisition).
  - **`serverInfo.version`:** available as `client.initializeResult?.serverInfo?.version` AFTER `await client.start()` (`InitializeResult.serverInfo = { name; version? }`).
- **Task 11 / V3 vsce+ovsx flags + GHA secret-guard (verified against installed pinned versions + current GHA docs):**
  - `@vscode/vsce` 3.9.2: `package --no-dependencies` (correct for an esbuild-bundled extension — the runtime dep is inlined into `dist/`); `publish` reads the PAT from the **`VSCE_PAT` env var by default** (no `-p` needed); `publish --packagePath <vsix>` publishes a pre-built VSIX (no version bump, no git tag).
  - `ovsx` 1.0.2: **`-p/--pat` is a GLOBAL (top-level) option, NOT a `publish` sub-flag**; ovsx auto-reads the **`OVSX_PAT`** env var (`node_modules/ovsx/lib/util.js:36`). Robust form: `npx ovsx publish ./*.vsix` with `OVSX_PAT` in the env. **The plan's human-owned secret NAME stays `OVSX_TOKEN`; it is surfaced to the CLI as the `OVSX_PAT` env var** (`env: OVSX_PAT: ${{ secrets.OVSX_TOKEN }}`).
  - GHA secret-guard: `secrets.*` CANNOT be used in a step-level `if:` — use job-level `env:` indirection (`env: { VSCE_PAT: ${{ secrets.VSCE_PAT }} }`) + step `if: env.VSCE_PAT != ''`; an unset secret resolves to `''` → the step skips. Publish legs stay DORMANT without secrets. Confirmed verbatim against GitHub's docs.
- **Task 11 / CI-trigger reality (corrects the plan's "push branch → CI runs" assumption):** the repo's `ci.yml` triggers are `push: branches: [main]` + `pull_request` ONLY. A **feature-branch push does NOT trigger CI**, and `workflow_dispatch` requires the trigger to exist on the DEFAULT branch (main). So the Task-11-Step-3 / DoD "confirm extension+extension-e2e green on GitHub Actions" is satisfied by the **finish-line merge-push to main** (which triggers `ci.yml` with all 6 jobs incl. the 2 new ones), NOT by a feature-branch push. Strong local evidence stands in pre-merge (all CI commands + the clean-dist e2e ran green locally). The e2e job's ubuntu deps were hardened with the ubuntu-24.04 `t64` package names. A **Critical** was caught + fixed here that WOULD have reddened main on the merge-push: the `extension-e2e` job now builds the bundle first (`pretest:e2e` hook) so `dist/extension.js` exists in a clean checkout.
- **Task 10 / V8 E2E harness shape (`@vscode/test-cli` 0.0.15 + `@vscode/test-electron` 3.0.0):** config file is `.vscode-test.mjs` exporting `defineConfig({...})`; the CLI runs Mocha itself over compiled-`.js` (`files: "out/e2e/**/*.test.js"`) — NO hand-written `runTest.ts`/`suite/index.ts`. `extensionDevelopmentPath` = the extension dir; the ExtensionApi is fetched via `vscode.extensions.getExtension("aaronbassett.compact-analyzer")?.activate()`. NO built-in settings field → the config WRITES `fixture/.vscode/settings.json` (abs `serverPath`) before launch; scenario 2 rewrites in-process via `config.update(..., ConfigurationTarget.Workspace)` + the `restartServer` command. `--user-data-dir=/tmp/...` (non-Windows) is REQUIRED to dodge the 104-char `sun_path` IPC-socket limit on deep checkout paths. **`@vscode/test-cli`/`test-electron` require Node ≥ 22** (the extension's own `.nvmrc`/`engines.node` is 20.19 for the runtime — the e2e leg needs a SEPARATE Node ≥ 22). The harness downloads a VS Code build at bootstrap (network needed, or a cached `.vscode-test/`). `test:e2e` = `tsc -p src/test/e2e/tsconfig.json && vscode-test`.
- **Task 9 / server-manifest.json pulled forward:** the plan assigned the dev-placeholder `server-manifest.json` to Task 11, but `activate()` must LOAD it, so Task 9 created it (64-zero sha256 placeholder, ships in the VSIX, loaded from `context.extensionPath`, parsed DEFENSIVELY → missing/malformed degrades to no-download, never throws). Task 11 still owns the `gen-server-manifest.mjs` generator + release regeneration.
- **Task 3 / deviation + deferred minors:** `[package.metadata.dist] dist = true` was added to the BIN crate `crates/compact-analyzer/Cargo.toml` (not root — the plan File-Structure only anticipated a root-Cargo.toml change) because `publish = false` hides the bin from dist discovery — necessary + minimal, no `publish = true` anywhere. DEFERRED minors (pre-tag polish, dormant until H4+tag): bin crate has no `description`/`homepage` → generated `compact-analyzer.rb` gets placeholder desc + repository-URL homepage fallback; add before the first real `v0.1.0` tag if formula copy matters.
