...
- **E2 (Task 4) — `push_file_items` offered only top-level items, never enclosing-module siblings.** Step 3's `push_file_items` iterated only `tree.top_level()`, which excludes items whose `parent` is `Some(..)` (i.e. module children) — while its own doc comment claimed "Top-level (and enclosing-module) items," and both the brief's Context line ("in-scope top-level/module items") and design spec §3.2 ("collects enclosing-module members") require module members to be offered. M2's `resolve_in_file_scope` DOES resolve enclosing-module siblings (before top-level), so completion under-offered: a reference to a sibling item inside `module M { … }` resolved via M2 but was never suggested. This is a completeness gap against the plan's own stated scope, NOT a deliberate deferral (contrast the string-path-import deferral, Self-review note 4). Root cause: the sample code implemented only the top-level half of `resolve_in_file_scope`. **Fix:** `push_file_items` now also finds the innermost enclosing `Module` (mirroring `enclosing_module` / `resolve_in_file_scope`: same `kind == Module && full_range.contains(offset)` predicate + `.next_back()` innermost selection, no `exported` gate for same-file children) and offers its `children_of` matching the `ItemFilter`, in addition to `top_level()`. Guarded by a `module { }` test proving both the positive (sibling offered) and the no-over-offer (a non-enclosing module's child stays absent) directions. Fixed in commit `561b5d7`.

- **E3 (Task 5) — `push_member` resolved the receiver at `receiver.text_range().start()`, which lands on leading whitespace, so `host.resolve` always returned `None`.** Step 3's `push_member` did `host.resolve(FilePosition { file, offset: receiver.text_range().start() })`. compactp attaches leading whitespace trivia as a node's first child, so an expression node's `text_range().start()` can point at a whitespace token rather than its head `IDENT` (the same trivia-attachment behavior recorded in M2a Errata E1, which drove the `render_ty`/`collapse_ws` helper). At a whitespace offset the resolver finds no identifier and returns `None`, so no ADT/enum members were ever offered — the task's own member tests failed RED. **Fix:** a `non_trivia_start(node)` helper returns the offset of the node's first non-trivia token, and `push_member` resolves the receiver there. Fixed in commit `b287d3a`. **NOTE for Task 6:** `ledger_method_hover` (hover.rs) uses the same `receiver.text_range().start()` pattern to resolve its receiver and is likely to need the same non-trivia-start fix.

- **E4 (Task 5) — the one-level-deep receiver boundary (spec §4) was neither tested nor actually enforced.** Spec §4 states the chained-receiver boundary ("`map.lookup(k).⎸` yields nothing, completing it would require `lookup`'s return type → v2") "is asserted by a test," but Task 5's test set omitted it. And `push_member` did not enforce the boundary: the original `receiver.text_range().start()` code only *appeared* to (it resolved nothing at all, via the E3 whitespace bug), while the E3 fix's `non_trivia_start` uses `descendants_with_tokens()` — which for a compound receiver (`CALL_EXPR`, e.g. `tbl.lookup(k).⎸` on a `Map` field) returns the innermost-leftmost IDENT (`tbl`), resolving the ledger field and offering its full `Map` ADT surface after `.lookup(k).` — a silent F7 violation surfaced by the Task-5 review. **Fix:** `push_member` now guards `receiver.kind() == SyntaxKind::NAME_EXPR` before resolving (a compound/`CALL_EXPR`/`INDEX_EXPR`/nested-`MEMBER_EXPR` receiver offers nothing), and the missing spec-§4 boundary test (`member_on_chained_receiver_offers_nothing`: `tbl.lookup(k).$0` on a `Map` field → none of `lookup`/`insert`/`member`/`isEmpty`/`size`/`remove` offered) was added. Fixed in commit `da89872`.

- **E5 (Task 6) — `ledger_method_hover` carried the same two receiver-resolution bugs as `push_member` (E3 + E4).** The brief's Step-3 `ledger_method_hover` resolved its receiver at `receiver.text_range().start()` (E3: lands on leading whitespace → `host.resolve` returns `None` → the task's own hover tests failed RED) and did not guard the receiver's node kind (E4: a compound/chained receiver like `tbl.lookup(k).member(x)` would resolve the inner `tbl` and surface `Map.member`'s hover). **Fix (commit `61c6a2d`):** resolve the receiver at its first non-trivia token (reused `completion::non_trivia_start`, promoted to `pub(crate)`) and guard `receiver.kind() == SyntaxKind::NAME_EXPR` before resolving. Added a chained-receiver hover boundary test (`tbl.lookup(k).member(k)` → no `Map.member` hover leak). Predicted by the "NOTE for Task 6" on E3 above.

- **E6 (Task 7) — the semantic-token classifier categorized `.` and `<`/`>` as `Operator`, contradicting the binding design spec §5.** Step 3's `is_operator` included `DOT`, `LT`, `GT`, while spec §5 requires `.` → the custom `punctuation` type and `<`/`>` → "classified by structural context (type-argument list vs relational operator)." Neither was among the plan's Self-review deliberate departures, so this was an oversight transcribing spec §5 into code, not a scoping choice — the human confirmed the spec governs. Additionally the resolution-refinement path (`classify_use_site`'s resolved branches) had zero committed coverage: the three plan tests all pass even if `classify_use_site` returns `fallback` unconditionally. **Fix (commit `dcd3b12`):** `DOT` moved to `is_punct` (`DOT_DOT`/`DOT_DOT_DOT` stay Operator); a dedicated `LT | GT` arm classifies by parent kind — `Punctuation` under `BYTES_TYPE`/`UINT_TYPE`/`VECTOR_TYPE`/`OPAQUE_TYPE`/`GENERIC_ARG_LIST`/`GENERIC_PARAM_LIST`/`DEFAULT_EXPR` (empirically confirmed via `compactp cst` to be the direct parents of the bracket tokens — never `TYPE_SIZE`), `Operator` under `BINARY_EXPR` (`LT_EQ`/`GT_EQ` always Operator); plus committed resolution-path tests (use-site ledger ref → Property/declaration:false; stdlib ref → default_library:true) that fail under a fallback-only `classify_use_site`.

- **E7 (Task 9 + cross-cutting) — `selection_ranges` panicked on an out-of-range offset.** `chain_at`'s `root.token_at_offset(offset)` relied on rowan returning `TokenAtOffset::None`, but rowan `assert!`-panics when `offset > root.text_range().end()` (it returns `None` only for an EMPTY node range). Since `offsets` is arbitrary caller input, a stale/past-EOF offset crashed the call — a never-die violation. **Fix (commit `2e4844c`):** guard `if offset > root.text_range().end() { return Vec::new(); }` per offset before `token_at_offset`, plus an out-of-range no-panic test and a multi-offset independence test. **NOTE (cross-cutting):** the same unguarded `token_at_offset` pattern exists in `completion.rs` and `hover.rs`; in the running server the binary's position→offset conversion clamps to the doc range and per-request `catch_unwind` backstops so the session survives, but the "answer empty/null" contract favors an explicit guard — addressed in the consolidated pre-merge fix wave.
