...
- **Known M2a simplifications (deliberate, recorded):** one flat namespace (no type-vs-value namespace split); rename conflict check is a single-site check at the definition's LAST reference (see Errata E3 — the plan originally said "definition-site only", but that anchor cannot detect a conflict with a later-declared binding and fails the plan's own `refuses_conflicting_target_name` test; full per-use-site conflict detection is deferred to M2b); references/rename are single-file; `IfStmt` has no condition accessor in compactp (irrelevant here — no bindings in conditions); export lists (`export { a, b };`) do not affect resolution visibility in M2a (everything in-file is visible; export only gates module-member imports).
...
Recorded per the M1 convention. Each entry is a defect in the code the plan *provided* — the plan's own tests would have failed against its own implementation until the noted fix. Later tasks that reuse these patterns must apply the same fix.

- **E1 (Task 3) — type-node `.text()` carries leading whitespace.** The brief stringified a param/const/generic type detail with `param.ty().map(|t| t.syntax().text().to_string())` (and the equivalent in `detail_for_binding`). Type nodes (`FIELD_TYPE`, `VECTOR_TYPE`, `TUPLE_TYPE`, …) include a leading-whitespace child token, so this yields e.g. `"base:  Field"` (double space) and fails `resolves_circuit_param` (expects `"base: Field"`). **Fix:** collapse whitespace on the rendered type via a `render_ty()` / `collapse_ws` helper (`split_whitespace().join(" ")`, which trims the spurious leading space without mangling internal separators in `Vector<10, Field>` / `[Field, Boolean]`). Applied at every type-stringification site. Any later task that renders a type from a syntax node (e.g. Task 4 file-scope item detail) must use the same collapsing helper, not raw `.text()`.
- **E3 (Task 7) — the rename conflict check cannot be anchored at the definition site.** The brief's `rename` did a single `resolve_name_at` at `def_name_range.start()` (the definition site) to detect whether `new_name` already resolves there. But Task 3's resolver treats a `const` as NOT visible before its own declaration (`resolve_local_name` breaks its block-walk the moment a `const` statement's range end exceeds the query offset). So for the plan's own test `refuses_conflicting_target_name` (`const a$0 = 1; const b = 2; return a + b;`, rename `a`→`b`), the check at `const a`'s offset never sees the later `const b` and returns `None` — no conflict — so the brief's literal code FAILS its own test. **Fix:** keep it a SINGLE `resolve_name_at` call (a per-use-site scan over all references is deliberately deferred to M2b), but anchor it at the LAST reference position (the max `name_range.start()` from `find_references`), where any later-declared binding of `new_name` is in scope. Verified in review: this detects the `a→b` conflict and does NOT fabricate a conflict for `base→input` (no such binding), and it does not disturb the earlier validation gates. Resolved per this plan's own principle that "the tests define the contract" (Self-review notes). Known residual limitation (tracked for M2b, consistent with the M2a scope split): a conflict visible only at a non-last reference inside a nested/shadowing scope is still missed — inherent to any single-site check. NOTE: this fix requires computing `find_references` BEFORE the conflict check, so the plan's literal validation order (`… → conflict → find_references`) is reversed to (`… → find_references → conflict`); this reordering is behaviorally inert for the `NotFound`-vs-`Conflict` precedence because `find_references` re-resolves the same already-resolved `pos` with no intervening host mutation.
- **E2 (Task 3) — `LAMBDA_EXPR`'s `PARAM_LIST` does not wrap children in `PARAM` nodes.** The brief resolved lambda params by iterating `lambda.param_list()?.params()` and calling `param_binding` (which assumes a `PARAM` node with `Param::pattern()`). Verified against compactp source: an *untyped* lambda param parses to a bare `IDENT_PAT` (no `PARAM` wrapper — invisible to `ParamList::params()`), and a *typed* lambda param wraps a bare `IDENT` token in `PARAM` (not an `IDENT_PAT` — invisible to `Param::pattern()`). The brief loop finds nothing and `resolves_lambda_param` fails. **Fix:** a dedicated `lambda_param_binding()` that walks the `PARAM_LIST`'s raw children with a three-way branch (`Pat::cast` → `Param::cast` + `pattern()` → bare-`IDENT`-token fallback). This is distinct from `CIRCUIT_DEF`/`CONSTRUCTOR_DEF` param binding, which *do* use `PARAM` nodes and are unaffected. Both branches were confirmed correct in review; the typed branch is currently unexercised by tests (Minor coverage gap, tracked in the ledger).
